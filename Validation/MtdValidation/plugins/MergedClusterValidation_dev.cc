...
void MergedClusterValidation_dev::beginJob() {
  ...
  // 2D histograms
  h_mc_energy_vs_time_ = fs->make<TH2F>(
  ...
  h_mc_energy_vs_nClusters_ = fs->make<TH2F>(
      "h_mc_energy_vs_nClusters_", "Energy vs N Clusters;N_{clusters};Energy [MeV]", 10, 0.5, 10.5, 100, 0, 50);
  h_merging_efficiency_ = fs->make<TH2F>(
      "h_merging_efficiency", "Merging Efficiency;Cluster Energy [MeV];Merged?", 50, 0, 50, 2, -0.5, 1.5);

  // efficiency
  h_eta_adjacent_pairs_ =
      fs->make<TH1F>("h_eta_adjacent_pairs", "Adjacent Pairs vs Eta;#eta;Number of Adjacent Pairs", 100, -1.5, 1.5);
  h_eta_merged_pairs_ =
      fs->make<TH1F>("h_eta_merged_pairs", "Merged Pairs vs Eta;#eta;Number of Merged Pairs", 100, -1.5, 1.5);
  h_eta_merging_efficiency_ =
      fs->make<TH1F>("h_eta_merging_efficiency", "Merging Efficiency vs Eta;#eta;Efficiency", 100, -1.5, 1.5);
  ...
  h_simmc_logEnergy_ =
      fs->make<TH1F>("h_simmc_logEnergy", "MergedCluster Log(Energy);Log(Energy [MeV]);Count", 100, -3, 3);
  ...
  h_simmc_logEnergy_perCluster_ =
      fs->make<TH1F>("h_simmc_logEnergy_perCluster", "Cluster Log(Energy);Log(Energy [MeV]);Count", 100, -3, 3);
  ...
  h_simmc_primaryPt_vs_energy_ =
      fs->make<TH2F>("h_simmc_primaryPt_vs_energy",
                     ...
                     0,
                     11);
  h_simmc_primaryEnergy_vs_energy_ = fs->make<TH2F>(
  ...
  h_simmc_primaryEnergy_vs_nClusters_ =
      fs->make<TH2F>("h_simmc_primaryEnergy_vs_nClusters",
...
void MergedClusterValidation_dev::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto* mcPtr : mergedVec) {
    ...
    if (evt_event_ <= 3) {
      std::cout << "MergedCluster: E=" << mc.energy() << " MeV, t=" << mc.time() << " ns, pos=(" << mc.x() << ","
                << mc.y() << ")" << std::endl;
      ...
      for (int etaOffset : etaOffsets) {
        ...
        if (hasOppositeEdgeHit) {
          ...
          if (evt_event_ <= 3 && adjacentPairs <= 10) {
            std::cout << "  ETA adjacent pair " << adjacentPairs << ": " << detId1 << " <-> " << detId2
...
#if DEBUG > 0
      std::cout << "\t\t Starting TP: pdgId = " << tp_ref->pdgId() << ", status = " << tp_ref->status()
                << ", E = " << tp_ref->energy() << " GeV"
                << ", #g4tracks = " << tp_ref->g4Tracks().size() << std::endl;
#endif
...
// No parent tracks - this is the root ancestor
...
#if DEBUG > 0
        std::cout << "\t\t Moving to parent: pdgId = " << current->pdgId() << ", status = " << current->status()
                  << std::endl;
#endif
      }
...
#if DEBUG > 0
      std::cout << "\t\t Final ancestor: pdgId = " << ancestor.pdgId() << ", status = " << ancestor.status()
                << ", E = " << ancestor.energy() << " GeV"
                << ", #g4tracks = " << ancestor.g4Tracks().size() << std::endl;
#endif
  ...
  if (evt_event_ <= 3) {
    std::cout << "Event " << evt_event_ << ": " << mc_n_ << " MergedClusters, " << cluster_n_ << " individual clusters"
              << std::endl;
    std::cout << "Adjacent pairs: " << adjacentPairs << ", Merged pairs: " << mergedPairs << std::endl;
  }
}
...
void MergedClusterValidation_dev::endJob() {
  ...
  if (totalAdjacentPairs_ > 0) {
    std::cout << "Pair merging efficiency: " << (double)totalMergedPairs_ / totalAdjacentPairs_ * 100 << "%"
              << std::endl;
  }
