...
private:
  ...
  std::vector<float> probPi_firstIteration_vec;

  std::vector<float> isPi_firstIteration_vec;
  std::vector<float> isP_firstIteration_vec;
  std::vector<float> isK_firstIteration_vec;
  std::vector<float> noPID_firstIteration_vec;
  std::vector<float> noPIDtype_firstIteration_vec;
  ...
  edm::EDGetTokenT<edm::ValueMap<float>> probPi_firstIterationToken_;
  edm::EDGetTokenT<edm::ValueMap<float>> probK_firstIterationToken_;
  edm::EDGetTokenT<edm::ValueMap<float>> probP_firstIterationToken_;
...
Primary4DVertexValidation::Primary4DVertexValidation(const edm::ParameterSet& iConfig)
  ...
  probPi_firstIterationToken_ = consumes<edm::ValueMap<float>>(iConfig.getParameter<edm::InputTag>("probPi_firstIteration"));
  probK_firstIterationToken_ = consumes<edm::ValueMap<float>>(iConfig.getParameter<edm::InputTag>("probK_firstIteration"));
  probP_firstIterationToken_ = consumes<edm::ValueMap<float>>(iConfig.getParameter<edm::InputTag>("probP_firstIteration"));
...
void Primary4DVertexValidation::MakeBranches() {
  ...
  //dump_tree->Branch("probPi_firstIteration",&probPi_firstIteration_vec);
  dump_tree->Branch("isPi_firstIteration",&isPi_firstIteration_vec);
  dump_tree->Branch("isP_firstIteration",&isP_firstIteration_vec);
  dump_tree->Branch("isK_firstIteration",&isK_firstIteration_vec);
  dump_tree->Branch("noPID_firstIteration",&noPID_firstIteration_vec);
  dump_tree->Branch("noPIDtype_firstIteration",&noPIDtype_firstIteration_vec);
...
void Primary4DVertexValidation::ClearVectors() {
  ...
  mass_vec.clear();
  sigmat0_vec.clear();
  probPi_firstIteration_vec.clear();
  isPi_firstIteration_vec.clear();
  isP_firstIteration_vec.clear();
  isK_firstIteration_vec.clear();
  noPID_firstIteration_vec.clear();
  noPIDtype_firstIteration_vec.clear();
...
void Primary4DVertexValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const auto& probPi_firstIteration = iEvent.get(probPi_firstIterationToken_);
  const auto& probK_firstIteration = iEvent.get(probK_firstIterationToken_);
  const auto& probP_firstIteration = iEvent.get(probP_firstIterationToken_);
      ...
      for (unsigned int iev = 0; iev < simpv.size(); iev++) {
        ...
        for (auto iTrack = vertex->tracks_begin(); iTrack != vertex->tracks_end(); ++iTrack) {
          ...
          if (tp_info != nullptr && matchCategory == 1) {
            ...
            unsigned int noPIDtype_firstIteration = 0;
            bool noPID_firstIteration = false, isPi_firstIteration = false, isK_firstIteration = false, isP_firstIteration = false;
            ...
            isParticle(*iTrack, sigmat0, sigmat0Safe, probPi_firstIteration, probK_firstIteration, probP_firstIteration, noPIDtype_firstIteration, noPID_firstIteration, isPi_firstIteration, isK_firstIteration, isP_firstIteration);
            ...
            isPi_firstIteration_vec.push_back(isPi_firstIteration);
            isP_firstIteration_vec.push_back(isP_firstIteration);
            isK_firstIteration_vec.push_back(isK_firstIteration);
            noPID_firstIteration_vec.push_back(noPID_firstIteration);
            noPIDtype_firstIteration_vec.push_back(noPIDtype_firstIteration);
	    ...
	    //selectTP_vec.clear();
            ...
            trackW_vec.push_back(vertex->trackWeight(*iTrack));
            sigmat0_vec.push_back(sigmat0[*iTrack]);
            probPi_firstIteration_vec.push_back(probPi_firstIteration[*iTrack]);
            isPi_firstIteration_vec.push_back(isPi_firstIteration);
            isP_firstIteration_vec.push_back(isP_firstIteration);
            isK_firstIteration_vec.push_back(isK_firstIteration);
            noPID_firstIteration_vec.push_back(noPID_firstIteration);
  ...
  for (unsigned int ir = 0; ir < recopv.size(); ir++) {
    if (recopv.at(ir).ndof > selNdof_) {
      ...
      singlevtx_t_vec.push_back(recopv.at(ir).recVtx->t());
      singlevtx_tError_vec.push_back(recopv.at(ir).recVtx->tError());
...
void Primary4DVertexValidation::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("probPi_firstIteration", edm::InputTag("tofPID:probPifirstIteration"));
  desc.add<edm::InputTag>("probK_firstIteration", edm::InputTag("tofPID:probKfirstIteration"));
  desc.add<edm::InputTag>("probP_firstIteration", edm::InputTag("tofPID:probPfirstIteration"));
