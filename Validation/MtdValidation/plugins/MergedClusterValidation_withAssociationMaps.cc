...
MergedClusterValidation_withAssociationMaps::MergedClusterValidation_withAssociationMaps(
  ...
  mergedSimToTPMap_ =
      consumes<reco::MergedSimToTPCollectionMtd>(iConfig.getParameter<edm::InputTag>("mergedSimToTPMap"));
  mergedTPToSimMap_ =
      consumes<reco::TPToMergedSimCollectionMtd>(iConfig.getParameter<edm::InputTag>("mergedTPToSimMap"));
...
void MergedClusterValidation_withAssociationMaps::beginJob() {
  ...
  h_deltaEnergy_ =
      fs->make<TH1F>("h_deltaEnergy", "Energy Resolution (Reco - Sim); #Delta E [MeV]; Entries", 80, -20, 20);
  ...
  h_deltaNclu_ = fs->make<TH1F>(
      "h_deltaNclu", "Difference in Cluster Multiplicity (Reco - Sim); #Delta N_{clusters}; Entries", 21, -10, 10);
  h_deltaNhits_ = fs->make<TH1F>(
      "h_deltaNhits", "Difference in Hit Multiplicity (Reco - Sim); #Delta N_{hits}; Entries", 41, -20, 20);
  h_nSimPerReco_ = fs->make<TH1F>("h_nSimPerReco",
                                  "Number of SimMergedClusters associated to each RecoMergedCluster; "
                                  "N_{SimMergedClusters} per Reco MergedCluster; Entries",
                                  10,
                                  0,
                                  10);

  h_deltaTime_multiClu_ = fs->make<TH1F>(
      "h_deltaTime_multiClu", "Time Resolution (Reco - Sim) Multi-Cluster;#Delta t [ns]; Entries", 50, -1, 1);
  h_deltaEnergy_multiClu_ = fs->make<TH1F>(
      "h_deltaEnergy_multiClu", "Energy Resolution (Reco - Sim) Multi-Cluster; #Delta E [MeV]; Entries", 80, -5, 5);
  h_deltaX_multiClu_ = fs->make<TH1F>(
      "h_deltaX_multiClu", "Local X Resolution (Reco - Sim) Multi-Cluster; #Delta X [cm]; Entries", 40, -10, 10);
  h_deltaY_multiClu_ = fs->make<TH1F>(
      "h_deltaY_multiClu", "Local Y Resolution (Reco - Sim) Multi-Cluster; #Delta Y [cm]; Entries", 50, -3, 3);

  h_deltaNclu_multiClu_ =
      fs->make<TH1F>("h_deltaNclu_multiClu",
                     "Difference in Cluster Multiplicity (Reco - Sim) Multi-Cluster; #Delta N_{clusters}; Entries",
                     21,
                     -10,
                     10);
  h_deltaNhits_multiClu_ =
      fs->make<TH1F>("h_deltaNhits_multiClu",
                     "Difference in Hit Multiplicity (Reco - Sim) Multi-Cluster; #Delta N_{hits}; Entries",
                     41,
                     -20,
                     20);
  h_nSimPerReco_multiClu_ = fs->make<TH1F>("h_nSimPerReco_multiClu",
                                           "Number of SimMergedClusters associated to each RecoMergedCluster "
                                           "Multi-Cluster; N_{SimMergedClusters} per Reco MergedCluster; Entries",
                                           10,
                                           0,
                                           10);

  h_deltaTime_singleClu_ = fs->make<TH1F>(
      "h_deltaTime_singleClu", "Time Resolution (Reco - Sim) Single-Cluster;#Delta t [ns]; Entries", 50, -1, 1);
  h_deltaEnergy_singleClu_ = fs->make<TH1F>(
      "h_deltaEnergy_singleClu", "Energy Resolution (Reco - Sim) Single-Cluster; #Delta E [MeV]; Entries", 80, -5, 5);
  h_deltaX_singleClu_ = fs->make<TH1F>(
      "h_deltaX_singleClu", "Local X Resolution (Reco - Sim) Single-Cluster; #Delta X [cm]; Entries", 40, -10, 10);
  h_deltaY_singleClu_ = fs->make<TH1F>(
      "h_deltaY_singleClu", "Local Y Resolution (Reco - Sim) Single-Cluster; #Delta Y [cm]; Entries", 50, -3, 3);

  h_deltaTime_vs_Eta_ =
      fs->make<TH2F>("h_deltaTime_vs_Eta", "Time Resolution vs Eta;#eta; #Delta t [ns]", 50, -1.5, 1.5, 50, -0.5, 0.5);
  h_deltaEnergy_vs_Eta_ = fs->make<TH2F>(
      "h_deltaEnergy_vs_Eta", "Energy Resolution vs Eta;#eta; #Delta E [MeV]", 50, -1.5, 1.5, 80, -20., 20.);
}
...
void MergedClusterValidation_withAssociationMaps::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& detSet : *mergedClustersHandle) {
    for (const auto& mc : detSet) {
      ...
      std::cout << "RECO MergedCluster with " << mc.clusterRefs().size() << " reco clusters has "
                << simMergedRefs.size() << " matches in SIM." << std::endl;
      std::cout << "E = " << mc.energy() << " MeV, t = " << mc.time() << " ns" << std::endl;
      ...
      std::cout << "  Resolutions: deltaT=" << deltaTime << " ns, deltaE=" << deltaEnergy << " MeV, deltaX=" << deltaX
                << " cm, deltaY=" << deltaY << " cm" << std::endl;
  ...
  for (const auto& simMergedCluster : *simMergedClustersHandle) {
    ...
    MtdSimMergedClusterRef simMergedClusterRef = edm::Ref<MtdSimMergedClusterCollection>(
        simMergedClustersHandle, &simMergedCluster - &(*simMergedClustersHandle->begin()));
    ...
    if (itp != mergedSimToTPMap.end()) {
      ...
      for (const auto& tpMatch : itp->val) {
        if (tpMatch.isNonnull()) {
          std::cout << "  TP MATCH : pdgId=" << tpMatch->pdgId() << ", pt=" << tpMatch->pt()
                    << " GeV, eta=" << tpMatch->eta() << ", phi=" << tpMatch->phi() << std::endl;
        } else {
  ...
  for (const auto& tp : *trackingParticlesHandle) {
    TrackingParticleRef tpRef =
        edm::Ref<TrackingParticleCollection>(trackingParticlesHandle, &tp - &(*trackingParticlesHandle->begin()));
    ...
    if (itp != mergedTPToSimMap.end()) {
      ...
      for (const auto& simMatch : itp->val) {
        if (simMatch.isNonnull()) {
          std::cout << "  Matched SimMergedCluster: E=" << convertUnitsTo(0.001_MeV, simMatch->simEnergy())
                    << " MeV, t=" << simMatch->simTime() << " ns" << std::endl;
        } else {
