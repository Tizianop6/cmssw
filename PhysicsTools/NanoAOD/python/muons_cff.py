...
muonTable = simplePATMuonFlatTableProducer.clone(
    ...
    variables = cms.PSet(CandVars,
        ...
        timeAtIpInOut    = Var("time().timeAtIpInOut",    float, doc="Muon time at IP in/out",    precision=10),
        timeAtIpInOutErr = Var("time().timeAtIpInOutErr", float, doc="Uncertainty on timeAtIpInOut", precision=10),
        timeAtIpOutIn    = Var("time().timeAtIpOutIn",    float, doc="Muon time at IP out/in",    precision=10),
        timeAtIpOutInErr = Var("time().timeAtIpOutInErr", float, doc="Uncertainty on timeAtIpOutIn", precision=10),
        ...
        rpcTimeAtIpInOut    = Var("rpcTime().timeAtIpInOut",    float, doc="RPC Muon time at IP in/out",    precision=10),
        rpcTimeAtIpInOutErr = Var("rpcTime().timeAtIpInOutErr", float, doc="RPC Uncertainty on timeAtIpInOut", precision=10),
        rpcTimeAtIpOutIn    = Var("rpcTime().timeAtIpOutIn",    float, doc="RPC Muon time at IP out/in",    precision=10),
        rpcTimeAtIpOutInErr = Var("rpcTime().timeAtIpOutInErr", float, doc="RPC Uncertainty on timeAtIpOutIn", precision=10),
        rpcDirection = Var("rpcTime().direction", "uint8", doc="Direction, OutsideIn = -1, Undefined = 0, InsideOut = 1, RPC"),
        rpcTimeNdof = Var("rpcTime().nDof", "uint8", doc="Number of measurements used in time calculation, RPC"),
        isTimeValid = Var("isTimeValid()", bool, doc="Time measurement is valid"),
        inverseBeta = Var("inverseBeta()", float, doc="1/beta", precision=10),
        inverseBetaErr = Var("inverseBetaErr()", float, doc="1/beta error", precision=10),
        ),
